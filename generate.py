...
def generate(lang,d):
    ...
    def head(title,intro,kicker):return f'<section class="page-head wrap"><div class="page-head-image"><img src="{base}assets/{'human' if kicker==d['nav'][4] else 'research' if kicker==d['nav'][1] else 'science'}.webp" alt="" width="1536" height="1024"></div><span class="eyebrow">{kicker}</span><h1>{title}</h1><p class="lead">{intro}</p></section>'
    ...
    for current,page in enumerate(pages):
        ...
        if page=='index':
            cards=''.join(f'<a class="focus-card" href="focus.html#area-{i}"><img class="focus-photo" src="{base}assets/{['human','science','research'][i]}.webp" alt="" loading="lazy" width="900" height="600"><div class="focus-card-copy"><span class="index">0{i+1}</span><h3>{t}</h3><p>{d["focusShort"][i]}</p><span class="card-arrow" aria-hidden="true">↗</span></div></a>' for i,t in enumerate(d['focusNames']))
            body=f'''<section class="hero"><div class="hero-shade"></div><div class="hero-copy"><span class="eyebrow">{d['eyebrow']}</span><h1>{d['hero']}</h1><p>{d['intro']}</p><a class="button" href="about.html">{d['discover']}<span aria-hidden="true">↗</span></a></div><div class="hero-art"><img src="{base}assets/science.webp" alt="" fetchpriority="high" width="1536" height="1024"><div class="image-caption">{'SCIENCE. PURPOSE. POSSIBILITY.' if lang=='en' else 'BİLİM. AMAÇ. OLASILIK.'}</div></div></section>
...
<section class="wrap section purpose"><figure class="purpose-photo"><img src="{base}assets/human.webp" alt="" loading="lazy" width="1536" height="1024"></figure><div><span class="eyebrow">03 / {'Our purpose' if lang=='en' else 'Amacımız'}</span><h2>{d['whyTitle']}</h2></div><div>{paras([d['why'],d['why2']])}</div></section>{cta()}'''
        elif page=='about':
